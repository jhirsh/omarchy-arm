...
| Arch Linux ARM rebuilds Arch's packages one at a time, with no staging | aquamarine was rebuilt with a new soname on a Tuesday and hyprland that links it was not, so for days nothing on any aarch64 mirror satisfied hyprland and pacman threw the whole 120-package transaction away after the prompt. The plan step now resolves the set with `pacman -Sp` first. What the repository cannot satisfy is covered in two tiers: a vendored known-good set (the lock set) installed with `pacman -U`, and, for anything not pinned, a build from Arch's own recipe at the tag x86_64 ships. The official build replaces either when it lands |
| No dated package archive to pin to, and mirrors keep only the latest | Arch's own `archive.archlinux.org` lets x86 pin a whole repository to a date; Arch Linux ARM has no equivalent and deletes superseded builds, so the coherent set from before a soname bump is already gone. There is nothing to pin *to*, so the fork pins by vendoring the built files themselves: `install/arm/pin-packages.sh` captures the installed versions of a named set out of pacman's cache on a working machine, checksums them into `install/arm/packages.pinned`, and uploads them to a release; `install.sh` installs that set when the live repository cannot. It is the lock set a distribution without an archive has to keep for itself. The recipe build and the lock set are one pipeline, not two: `build_from_arch` leaves what it compiles in pacman's cache, so `pin-packages.sh` vendors exactly the artifact that was built and verified, and a later board installs that binary instead of compiling it again |
